...
def test_initial_liquidity(alice, swap, wrapped_coins, pool_token, min_amount, wrapped_decimals):
    ...
    assert pool_token.balanceOf(alice) == len(wrapped_coins) * 10**18
    assert pool_token.totalSupply() == len(wrapped_coins) * 10**18


@pytest.mark.parametrize("idx", range(4))
def test_initial_liquidity_missing_coin(alice, swap, pool_token, idx, wrapped_decimals):
    if idx >= len(wrapped_decimals):
        return
    amounts = [10**i for i in wrapped_decimals]
    amounts[idx] = 0
