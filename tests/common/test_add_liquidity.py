...
def test_add_liquidity(bob, swap, wrapped_coins, pool_token, initial_amounts):
    ...
    assert pool_token.balanceOf(bob) == len(initial_amounts) * 10**24
    assert pool_token.totalSupply() == len(initial_amounts) * 10**24 * 2


def test_add_liquidity_with_slippage(bob, swap, wrapped_coins, pool_token, wrapped_decimals):
    ...
    assert 0.999 < pool_token.balanceOf(bob) / (len(amounts) * 10**18) < 1


@pytest.mark.itercoins("idx")
def test_add_liquidity_one_coin(bob, swap, wrapped_coins, pool_token, initial_amounts, idx):
    if idx >= len(wrapped_coins):
        raise

    amounts = [0] * len(wrapped_coins)
...
def test_insufficient_balance(charlie, swap, wrapped_coins, pool_token, wrapped_decimals):
    amounts = [(10**i) for i in wrapped_decimals]
...
def test_min_amount_too_high(alice, swap, wrapped_coins, pool_token, min_amount, wrapped_decimals):
    amounts = [10**i for i in wrapped_decimals]
    with brownie.reverts():
        swap.add_liquidity(amounts, min_amount, {'from': alice})


def test_min_amount_with_slippage(bob, swap, wrapped_coins, pool_token, wrapped_decimals):
    amounts = [10**i for i in wrapped_decimals]
    amounts[0] = int(amounts[0] * 0.99)
    amounts[1] = int(amounts[1] * 1.01)
    with brownie.reverts("Slippage screwed you"):
        swap.add_liquidity(amounts, len(amounts) * 10**18, {'from': bob})
